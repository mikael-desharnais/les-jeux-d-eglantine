  ...
  .directive('webcam', function () {
    return {
      ...
      link: function postLink($scope, element) {
        ...
        $scope.config.snap = function(){
        	var hiddenCanvas = document.createElement('canvas');
		    hiddenCanvas.width = jQuery($scope.config.video).width();
		    hiddenCanvas.height = jQuery($scope.config.video).height();
		    var ctx = hiddenCanvas.getContext('2d');
		    ctx.drawImage($scope.config.video, 0, 0, jQuery($scope.config.video).width(), jQuery($scope.config.video).height());
        ...
        var startWebcam = function startWebcam() {
          ...
          // Default variables
          console.log(element,jQuery(element).width());
          var isStreaming = false,
            width = element.width = $scope.config.videoWidth || '100%',
